...
$inhouse += $Context.TenantId # Add the current tenant ID to the list of tenants to consider in-house
...
$Applications = Get-MgApplication -All | Where-Object { $_.RequiredResourceAccess.ResourceAppId -contains $APIobjects.AadGraph.AppId } | Select-Object Id, DisplayName, AppId, RequiredResourceAccess
...
foreach ($sp in $serviceprincipals) {
    ...
    if ($sp.appid -in $applications.appid) { # Determine if the SP has an App Registration in the tenant and get permissions from the manifest
    ...
    $DelegatedPermissions = Get-MgServicePrincipalOauth2PermissionGrant -ServicePrincipalId $sp.Id | 
        Where-Object { $_.ResourceId -in $ApiObjects.Values.AppId } | 
